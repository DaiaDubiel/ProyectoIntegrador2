...
marco.place(x=20,y=20,width=1000,height=700)
...
#botones
btnNuevo = Button(marco,text="Nuevo", pady=10, padx=10, command=lambda:nuevo(), bg="green", font =("Arial", 12, "bold"), fg="white", cursor = "hand2" )
btnNuevo.grid(column=0, row=10)

btnEliminar = Button(marco,text="Eliminar", pady=10, padx=10, command=lambda:eliminar(), bg="red", font =("Arial", 12, "bold"), fg="white", cursor = "hand2")
btnEliminar.config(state = "disabled")
btnEliminar.grid(column=1, row=10)

btnModificar = Button(marco,text="Modificar", pady=10, padx=10, command=lambda:actualizarALbum(), bg="blue", font =("Arial", 12, "bold"), fg="white", cursor = "hand2")
...
btnGuardar = Button(marco,text="Guardar", pady=10, padx=10, command=lambda:Guardar(), bg="green", font =("Arial", 12, "bold"), fg="white", cursor = "hand2" )
btnGuardar.config(state = "disabled")
btnGuardar.grid(column=0, row=13)

btnCancelar = Button(marco,text="Cancelar", pady=10, padx=10, command=lambda:[limpiar(),mostrarAlbum()], bg="red", font =("Arial", 12, "bold"), fg="white", cursor = "hand2")
btnCancelar.config(state = "disabled")
btnCancelar.grid(column=1, row=13)

btnConsultar = Button(marco,text="Consultar", pady=10, padx=10, command=lambda:buscaralbum(), bg="grey", font =("Arial", 12, "bold"), fg="white", cursor = "hand2" )
...
def validar():
    #return len(id_discografia.get()) and len(cod_album.get()) and len(nombre.get()) and len(cant_temas.get()) and len(fecha_lanzamiento.get()) and len(precio.get()) and len(cantidad.get()) 
    
    return len(cod_album.get()) and len(nombre.get()) and len(fecha_lanzamiento.get()) 
